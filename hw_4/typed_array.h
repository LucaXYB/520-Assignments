...
/*push*/
template <typename ElementType>
void TypedArray<ElementType>::push(const ElementType &val)
{
    set(size(), val);
}
...
/*push_front*/
template <typename ElementType>
void TypedArray<ElementType>::push_front(const ElementType &val)
{
    if (origin == 0)
    {
        extend_buffer();
...
TypedArray<ElementType> &TypedArray<ElementType>::reverse()
{
    ...
    for (int i = 0; i < len; ++i)
    {
        std::swap(buffer[index_to_offset(i)], buffer[index_to_offset(len - i - 1)]);
    }
    return *this;
}
